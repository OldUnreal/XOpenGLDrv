...
void UXOpenGLRenderDevice::DebugCallback(unsigned int source, unsigned int type, unsigned int id, unsigned int severity, int length, const char* message, const void* userParam)
{
	...
	switch (severity)
	{
		case GL_DEBUG_SEVERITY_LOW:
			if (LogLevel >= 2)
				debugf(NAME_DevGraphics, TEXT("XOpenGL DebugLog: In source %ls, type %ls, id %i, severity : LOW, message %ls "),*SourceMessage, *TypeMessage, id, appFromAnsi(message));
			break;
		case GL_DEBUG_SEVERITY_MEDIUM:
			if (LogLevel >= 1)
				debugf(NAME_DevGraphics, TEXT("XOpenGL DebugLog: In source %ls, type %ls, id %i, severity : MEDIUM, message %ls "),*SourceMessage, *TypeMessage, id, appFromAnsi(message));
			break;
		case GL_DEBUG_SEVERITY_HIGH:
			if (LogLevel >= 0)
				debugf(NAME_DevGraphics, TEXT("XOpenGL DebugLog: In source %ls, type %ls, id %i, severity : HIGH, message %ls "),*SourceMessage, *TypeMessage, id, appFromAnsi(message));
			break;
		default:
			if (LogLevel >= 3)
				debugf(NAME_DevGraphics, TEXT("XOpenGL DebugLog: In source %ls, type %ls, id %i, severity : NONE, message %ls "),*SourceMessage, *TypeMessage, id, appFromAnsi(message));
			break;
	}
}
