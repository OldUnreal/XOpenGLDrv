...
uniform struct FogParameters
{
	...
	float FogEnd;		// Oonly for linear fog
   ...
   if ( TextureFormat == TEXF_RGTC_RG) //BC5 (GL_COMPRESSED_RG_RGTC2) compression
	...
	if ((DrawFlags & DF_BumpMap) == DF_BumpMap)
	{
		...
		for(int i=0; i<NumLights; ++i)
		{
			...
			vec3 LightPosition_cameraspace = ( vviewMat * vec4(InLightPos,1)).xyz;
	...
	if ((DrawFlags & DF_FogMap) == DF_FogMap)
	{
        ...
        #ifdef BINDLESSTEXTURES
        if (TexNum[5] > uint(0))
            FogColor = texture(Textures[TexNum[5]], vFogMapCoords)*2.0;
        else FogColor = texture(Texture5, vFogMapCoords)*2.0;
        #else
		FogColor = texture(Texture5, vFogMapCoords)*2.0;
		#endif
        if ( (PolyFlags&PF_Masked) == PF_Masked )
        {
            if(FogColor.a < 0.5)
                discard;
            else FogColor.rgb /= FogColor.a;
        }
        else if ( (PolyFlags&PF_AlphaBlend) == PF_AlphaBlend && FogColor.a < 0.01 )
            discard;
