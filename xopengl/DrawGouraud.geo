...
in mat4 vmodelMat[];
in mat4 vviewMat[];
in mat4 vFrameCoords[];
//in mat4 vFrameUncoords[];
...
void main(void)
{
	...
	for(int i=0; i<3; ++i)
	{
		mat4 vmodelviewprojMat = vprojMat[i] * vviewMat[i] * vmodelMat[i];

		vec3 T = normalize(vec3(vmodelMat[i] * vec4(Tangent,0.0)));
		vec3 B = normalize(vec3(vmodelMat[i] * vec4(Bitangent,0.0)));
		vec3 N = normalize(vec3(vmodelMat[i] * vNormals[i]));
