...
// Light information. Need to do and calculate here since it seems not possible to pass that many variables from vertexshader! :(
...
void main(void)
{
	...
	if ((gDrawFlags & DF_BumpMap) == DF_BumpMap)
	{
		...
		for(int i=0; i<NumLights; ++i)
		{
			...
			vec3 LightPosition_cameraspace = ( gviewMat * vec4(InLightPos,1)).xyz;
	...
	if ((DistanceFogValues.w == 0.0 && (DistanceFogValues.y > DistanceFogValues.x)) || (DistanceFogValues.w > 0.0 && (DistanceFogValues.z > 0.0 )))
	{
		...
		float FogCoord = abs(gEyeSpacePos.z/gEyeSpacePos.w);
